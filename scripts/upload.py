...
for ip in robotsIP:
	if(not is_valid_ipv4(ip)):
		...
		try:
			...
			print '\033[1;31m Waited for ' + str(timeouttime) +  ' seconds. Robot with IP ' + ip + ' is unreachable \033[1;m'
			reachable = False
		sock.close()

		if(reachable):
			print "\033[1;32m Robot with IP " + ip + " is reachable\033[1;m (ssh ok)"
