...
bool NaoCamera::captureNew()
{
  ...
  if(polled < 0)
  {
   std::cout<<"NaoCamera: Cannot poll. Reason: " << strerror(errno)<<std::endl;
    assert(false);
  }
  else if(polled == 0)
  {
    std::cout<<("NaoCamera: 0.5 seconds passed and there's still no image to read from the camera. Terminating.");
     assert(false);
  }
  else if(pollfd.revents & (POLLERR | POLLNVAL))
  {
    std::cout<<("NaoCamera: Polling failed.");
     assert(false);
  }
...
void NaoCamera::initSelectCamera(  unsigned char camera)
{
  ...
  std::cout<<"GOING TO CAM:"<<camera<<std::endl;
  closeI2CAdapter(i2cfd);
#endif
  currentCamera = camera;
}
