...
public:
    ...
    void below(TopicTreeNode<Key,Data>* target_node,list<Data*>* result)
    {
        if (result == 0)
            return ;
        if ((target_node != 0) && (target_node->get_children().size() > 0))
        {
            for ( typename std::list< TopicTreeNode<Key, Data>* >::iterator child_it = target_node->get_children().begin();
    ...
    list<Data*>* message_arrived(const Key& k)
    {
        ...
        result->insert(result->end(),topic_node->get_data(ON_TOPIC).begin(),topic_node->get_data(ON_TOPIC).end());
        result->insert(result->end(),topic_node->get_data(BELOW_TOPIC).begin(),topic_node->get_data(BELOW_TOPIC).end());
        result->insert(result->end(),topic_node->get_data(ABOVE_TOPIC).begin(),topic_node->get_data(ABOVE_TOPIC).end());
