...
void KccLabel::updateLabel(VisionDebugMessage vdm) {
	...
	if(latestTimestamp - boost::posix_time::milliseconds(300) < boost::posix_time::from_iso_string(vdm.image_timestamp()) || 
		...
		if(vdm.has_ball_circle()) {
			...
			pen.setWidth(5);
			painter.setPen(pen);
			painter.drawEllipse(vdm.ball_circle().center().x()-vdm.ball_circle().radius(), vdm.ball_circle().center().y()-vdm.ball_circle().radius(), 
								2*vdm.ball_circle().radius(), 2*vdm.ball_circle().radius());
		}
