...
/* AttackerPenalty Initialization */
...
void AttackerPenalty::UserInit()
{
	...
	firstShoot = false;
	Logger::Instance().WriteMsg("AttackerPenalty", "Initialized: My number is " + _toString(playerNumber) + " and my color is " + _toString(teamColor), Logger::Info);
}


/* AttackerPenalty Main Execution Function */

int AttackerPenalty::Execute()
{
	read_messages();
	GetGameState();
	GetPosition();
	...
	if (gameState == PLAYER_PLAYING && kickoff)
	{
		CheckForBall();
...
void AttackerPenalty::GetGameState()
{
	if (gsm != 0)
	{
		//Logger::Instance().WriteMsg("AttackerPenalty", " Player_state " + _toString(gsm->player_state()), Logger::ExtraExtraInfo);
...
void AttackerPenalty::GetPosition()
{
	if(wim.get() != 0)
	{
...
void AttackerPenalty::UpdateOrientation()
{
	...
	//Logger::Instance().WriteMsg("AttackerPenalty", "OPPGOALX " + _toString(oppGoalX[teamColor]) + "OPPGOALY " + _toString(oppGoalY[teamColor]) + "OGB " + _toString(ogb)+ "ORIENTATION " + _toString(orientation), Logger::Info);
}


void AttackerPenalty::UpdateOrientationPlus()
{
...
void AttackerPenalty::CheckForBall()
{
...
int AttackerPenalty::MakeTrackBallAction()
{
	hmot->set_command("setHead");
...
void AttackerPenalty::HeadScanStepRaster()
{
...
void AttackerPenalty::HeadScanStepHigh(float yaw_limit)
{
	hmot->set_command("setHead");
...
void AttackerPenalty::HeadScanStepSmart()
{
...
void AttackerPenalty::HeadScanStepIntelligent()
{
	float bearing;
...
float AttackerPenalty::lookAtPointYaw(float x, float y)
{
	return anglediff2(atan2(y - robot_y, x - robot_x), robot_phi);
}


float AttackerPenalty::lookAtPointPitch(float x, float y)
{
...
void AttackerPenalty::Kick(int side)
{
	if (by > 0.0)
...
void AttackerPenalty::velocityWalk(double ix, double iy, double it, double f)
{
	double x, y, t;
	x = ix;
	y = iy;
...
void AttackerPenalty::littleWalk(double x, double y, double th)
{
...
void AttackerPenalty::approachBall(double ballX, double ballY)
{
...
void AttackerPenalty::approachBallNewWalk(double ballX, double ballY)
{
	static double X = 0.0, Y = 0.0, t = 0.0, f = 1.0, gain = 1.0;
	double maxd = fmaxf( fabs(bx - ballX), fabs(by - ballY) );
	f    = fminf(1.0, 0.4 + (maxd / 0.5));
...
void AttackerPenalty::gotoPosition(float target_x, float target_y, float target_phi)
{
...
void AttackerPenalty::calibrate()
{
	KCalibrateCam v;
	v.set_status(0);
...
bool AttackerPenalty::readConfiguration(const std::string& file_name)
{
	XMLConfig config(file_name);

	if (!config.QueryElement("player", playerNumber))
		Logger::Instance().WriteMsg("AttackerPenalty", "Configuration file has no player, setting to default value: " + _toString(playerNumber), Logger::Error);
	...
	if (!config.QueryElement("default_team_color", color))
		Logger::Instance().WriteMsg("AttackerPenalty", "Configuration file has no team_color, setting to default value: " + color, Logger::Error);
	...
	if (color == "blue")
		...
		Logger::Instance().WriteMsg("AttackerPenalty", "Undefined color in configuration, setting to default value: " + color, Logger::Error);

	return true;
}


bool AttackerPenalty::readRobotConfiguration(const std::string& file_name)
{
	if ( (playerNumber < 1) || (4 < playerNumber) )
	{
		Logger::Instance().WriteMsg("AttackerPenalty",  " readRobotConfiguration: Invalid player number "  , Logger::Error);
		return false;
	}
	...
	Logger::Instance().WriteMsg("AttackerPenalty",  " readRobotConfiguration "  , Logger::Info);
	...
	for (int i = 0; i < 2; i++)
	{
		...
		for (NodeCont::iterator it = robotPosition.begin(); it != robotPosition.end(); it++)
		{
			if (it->attrb["number"] == playerNumber)
			{
				...
				Logger::Instance().WriteMsg("AttackerPenalty", " readConf TEAM_BLUE INIT X " + kickoff + " " + _toString(initX[i][TEAM_BLUE]) + " INITY " + _toString(initY[i][TEAM_BLUE]) + " INITPHI " + _toString(initPhi[i][TEAM_BLUE]), Logger::Info);
				Logger::Instance().WriteMsg("AttackerPenalty", " readConf TEAM_RED INIT X " + kickoff + " " + _toString(initX[i][TEAM_RED]) + " INITY " + _toString(initY[i][TEAM_RED]) + " INITPHI " + _toString(initPhi[i][TEAM_RED]), Logger::Info);
				found = true;
			}
		}
		...
		if (!found)
		{
			Logger::Instance().WriteMsg("AttackerPenalty",  " readRobotConfiguration: Unable to find initial " + kickoff + " position for player number " + _toString(playerNumber) , Logger::Error);
			readRobotConf = false;
		}
	}
...
bool AttackerPenalty::readGoalConfiguration(const std::string& file_name)
{
	...
	if (!loadOkay)
	{
		Logger::Instance().WriteMsg("AttackerPenalty",  " readGoalConfiguration: cannot read file " + file_name , Logger::Info);
		return false;
	}
...
void AttackerPenalty::test()
{
	...
	if (ballfound == 1)
	{
		Logger::Instance().WriteMsg("AttackerPenalty",  " timestamp " + to_simple_string(microsec_clock::universal_time()), Logger::Info);
		float kickslope = -4;
		float x1, y1;
		float ubx, uby;
		float x, y, slope, t;
		...
		if(wim != 0)
		{
			if (wim->balls_size() > 0)
			{
				...
				Logger::Instance().WriteMsg("AttackerPenalty",  "OP: x1 " + _toString(x1) + " y1 " + _toString(y1), Logger::Info);
				Logger::Instance().WriteMsg("AttackerPenalty",  "OP: ubx " + _toString(ubx) + " uby " + _toString(uby), Logger::Info);
				...
				Logger::Instance().WriteMsg("AttackerPenalty", "OP: x " + _toString(x) + " y " + _toString(y) + "TIME: " + _toString(t), Logger::Info);
	...
	//if (om!=0) {
	//Logger::Instance().WriteMsg("AttackerPenalty", "L: " + _toString(om->direction(0)) + " C: " + _toString(om->direction(1)) + " R: " + _toString(om->direction(2)), Logger::Info);
