...
namespace {
  SpecialActionRegistrar<RightDive>::Type temp("RightDive");
}

RightDive::RightDive() {
  Initialize();
  Logger::Instance().WriteMsg("SP:RightDive","Initialized!",Logger::ExtraInfo);
  
  try {
    motion = KAlBroker::Instance().GetBroker()->getMotionProxy();
  } catch (AL::ALError& e) {
    Logger::Instance().WriteMsg("SP:RightDive","Error in getting motion proxy",Logger::FatalError);
  }
...
int RightDive::ExecutePost() {
  return motion->post.angleInterpolationBezier(RightDive_names, RightDive_times, RightDive_keys); 
} 
