...
public:
   ...
   void LinearInterpolationINIT(std::vector<float> &vec ,float start ,float end ,float Ts,float Tstep){
	...
	w2[j]=zdoti[j];
	w3[j]=zddoti[j]/2;
	w4[j]=10*((zf[j]-zi[j])/(pow(T,3))- zdoti[j]/pow(T,2) -zddoti[j]/(2*T))
...
w6[j]=6*((zf[j]-zi[j])/pow(T,5) - zdoti[j]/pow(T,4) -zddoti[j]/pow(T,3))
	...
	for (float t=0;t<=T;t+=Ts)
	{
	     ...
	     if (t<T/2) vec.push_back(w6[1]*pow(t,5)+w5[1]*pow(t,4)+w4[1]*pow(t,3)+w3[1]*pow(t,2)+w2[1]*t+w1[1]);
	     else vec.push_back(w6[2]*pow(t,5)+w5[2]*pow(t,4)+w4[2]*pow(t,3)+w3[2]*pow(t,2)+w2[2]*t+w1[2]);
