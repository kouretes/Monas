...
int FootTrajectoryPlanner::FootTrajectoryInterpolate(float Buffer[3][2][MAX_TRAJECTORY_LENGTH], int append )
{
	...
	if (RIGHTLEGSUPPORT)
	{
	    ...
	    KWalkMath.CubicSplineInterpolation(Buffer[Z][LEFT], --fillcounter[Z][LEFT] ,0.000,0.015/3,0.015,0.015/3,0.000,Robot.getWalkParameter(Ts),Robot.getWalkParameter(Tss));
	...
	else
	{
	    ...
	    KWalkMath.CubicSplineInterpolation(Buffer[Z][RIGHT], --fillcounter[Z][RIGHT],0.000,0.015/3,0.015,0.015/3,0.000,Robot.getWalkParameter(Ts),Robot.getWalkParameter(Tss));
