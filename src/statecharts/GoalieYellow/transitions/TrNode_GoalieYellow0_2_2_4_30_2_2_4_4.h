...
|| robotInPosition(behavior.Data.WorldInfo, behavior.State.PositionMessage) */
		...
		Logger::Instance().WriteMsg("_open_option_GoToPosition_one_or_more_times__close_option__TO_0.2.2.4.4, worldstate.State.GameStateMessage== NULL || worldstate.State.GameStateMessage.player_state()!=PLAYER_READY  || robotInPosition(behavior.Data.WorldInfo, behavior.State.PositionMessage)" ,_toString(var_1263977940.get()==0 ||  (var_1263977940.get()!=0 && var_1263977940->player_state()!=PLAYER_READY)  || robotInPosition(var_1071592760, var_1868674971)),  Logger::Info);
