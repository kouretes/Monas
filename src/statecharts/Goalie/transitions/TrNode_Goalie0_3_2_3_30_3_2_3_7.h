...
public:
	...
	bool Eval() {
		/* behavior.State.HeadToBMessage.ballfound()>0 || behavior.State.GameStateMessage.player_state()!=PLAYER_PLAYING || behavior.State.GameStateMessage==NULL */
		...
		if ( var_621149599.get() == 0 )
 		...
 		if ( var_1901744185.get() != 0  ){	
				return ( var_1901744185->ballfound()>0 || var_621149599->player_state()!=PLAYER_PLAYING  );
		}return false;
