...
public:
	...
	bool Eval() {
		/* behavior.State.GameStateMessage==NULL|| behavior.State.GameStateMessage.player_state()!=PLAYER_PLAYING || behavior.State.HeadToBMessage.ballfound()==0 || !ballAway(behavior.Data.WorldInfo) ||  behavior.State.FallMessage.fall()!=0 */
		...
		if ( var_621149599.get() == 0 )
			 return true;
 		if ( var_1901744185.get() != 0  && var_1071592760.get() != 0 && var_1561961811.get() != 0 ){	
				return (  var_621149599->player_state()!=PLAYER_PLAYING || var_1901744185->ballfound()==0 || !ballAway(var_1071592760) ||  var_1561961811->fall()!=0 );
		}return false;
