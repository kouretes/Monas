function [] = inverseRightHand()
close all;
%clear all;
clc;
%% Nao kinematics for the right arm
...
function [iRight] = inverseRight(px,py,pz,rx,ry,rz)
global shoulderOffsetY
global shoulderOffsetZ
global elbowOffsetY
global LowerArmLength
...
updown = up/down;
real(acos(updown));
if(real(acos(updown)) - pi/2 <= 0.3142 && real(acos(updown)) - pi/2 >= -1.3265)
...
inverseRight(Tendend(1,4),Tendend(2,4),Tendend(3,4),rotX,rotY,rotZ)
right = [Tendend(1:3,4);rotZ;rotY;rotX];
end
