...
void Segmentator::setLumaScale(float s)
{
	lumascale=s;
}
void Segmentator::yuv2hsy(unsigned char yuv[3], unsigned  int hsy[3])
{
 ...
 */ 
colormask_t Segmentator:: classifyPixel(unsigned char yuv[3])
{
	//return calculatePixel(yuv);
 ...
 */
colormask_t Segmentator:: calculatePixel(unsigned char yuv[3])
{
	unsigned int hsy[3];
	int dists,distl;
	Segmentator::yuv2hsy(yuv,hsy);
 ...
 */
colormask_t Segmentator::  classifyPixel(IplImage*data, int i,int j,int type)
{
 ...
 */
Segmentator::Segmentator(std::ifstream &conf) 
{
	...
	{
		cout<<"Segmentator(): Invalid configuration file"<<endl;
		return;
	}
	if(set.size-'0'>sizeof(colormask_t))
	{
		cout<<"Segmentator(): Not enough length in colormask_t"<<endl;
		return;
	}
...
void Segmentator::readCalibration(ifstream & conf){
	for(int i=0;i<set.calLines-'0';i++)
		conf.ignore(256,'\n');
	
}
void Segmentator::readColorInfo(ifstream & conf){
		for(int i=0;i<set.colorLines-'0';i++)
		conf.ignore(256,'\n');
	
}
	
void Segmentator::readRulefile(ifstream & conf){
	if(set.conf[0]=='H' && set.conf[1]=='S' && set.conf[2]=='Y' && set.conf[3]=='\n')
	{
		...
		cout<<"Segmentator():Invalid or unknown rule file header"<<endl;
	
}
	
void Segmentator::readColorTable(ifstream & conf){
