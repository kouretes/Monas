...
#try:
#alsentinel = config.loadProxy("ALSentinel")
#except Exception,e:
...
print motionProxy.getJointNames('Body')
...
while 1:
	...
	print( "We have opened a connection with: " + str( details ) + " succesfull")
	...
	while True:
		...
		try:
			...
			if "stiffness_on_all" in str(data) :
				...
				try:
					...
					for pose in lines:
						values = []
						strvalues = []
						strvalues = pose.split('%')

						#print "line values: " + line
						print pose
						strvalues.pop( 0 ) #delete "play" or "pose"
						duration = float(strvalues.pop()) # get duration and delete it from the list
						print "duration" + str(duration)
						print
						values = strvalues
						for index in range(len(strvalues)):
							values[index] = float(strvalues[index])

						motionProxy.angleInterpolation("Body",values,duration,True)
				except Exception, e:
					raise e
					print "EXCEPTION "
					continue
				...
				#motionProxy.gotoBodyAngles(values,duration+1,motion.INTERPOLATION_LINEAR)
			...
			sendValues+='\n\0'
			#print sendValues + "len pose " +str(len(pose))
